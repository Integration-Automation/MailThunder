...
Cross-repo and workspace items live in `D:\Codes\progress.md` (relevant here: S-8, X-7, X-11, X-12, L-8).

## Open

- **#1** Security: the executor registers every builtin, including `eval`, `exec` and `open`, as an action (`je_mail_thunder/utils/executor/action_executor.py:40-42`), so a JSON script can call any builtin. Replace it with an allowlist (compare LoadDensity's `_UNSAFE_BUILTINS` and APITestka's allowlist; workspace S-8, X-12).
