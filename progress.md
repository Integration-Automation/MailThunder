...
- **#7** The socket server's default port 9944 is also FileAutomation's HTTP action-server default, and the server also takes host/port overrides from `sys.argv`. The action-dict key is `auto_control` (renaming it would break stored action files, so it needs an alias period).
