"""Where Mail Thunder's log goes, and that importing the package writes nothing (workspace item X-6)."""
import logging
import os
...
from je_mail_thunder.utils.logging import loggin_instance
from je_mail_thunder.utils.logging.loggin_instance import LOG_FILE_ENV, MailThunderFileHandler, default_log_file
...
def test_default_is_under_the_home_directory(monkeypatch, tmp_path):
    ...
    assert default_log_file() == tmp_path / ".je_mail_thunder" / "logs" / "Mail_Thunder.log"
...
def test_importing_writes_no_file(tmp_path):
    target = tmp_path / "home" / "Mail_Thunder.log"
...
def test_first_record_creates_the_directory_and_later_handlers_append(tmp_path):
    target = tmp_path / "nested" / "Mail_Thunder.log"
    _log(MailThunderFileHandler(str(target)), "first")
    _log(MailThunderFileHandler(str(target)), "second 中文 ⠐ \udcff")
...
def test_an_unopenable_path_only_warns(tmp_path):
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        _log(MailThunderFileHandler(str(blocker / "Mail_Thunder.log")), "goes nowhere")
